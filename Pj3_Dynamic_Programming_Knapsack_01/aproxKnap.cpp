...
void aproxKnap(int n, int capacity, double beta, vector<pair<int,int>> objects){
	...
	//Calculamos X
	double x = (((double)1-beta)*(double)totalValue/(double)n);

	//Dividimos todos los valores por X
	totalValue = 0;
	for (int i = 0; i < n; ++i)
	{
	  objects[i].second = (int)((double)objects[i].second/x);
		totalValue += objects[i].second;
	}
...
void leerArchivo(int *n, int *c, double *beta,vector<pair<int,int>> *objects){
...
int main(){
	...
	leerArchivo(&n, &capacity, &beta, &objects);
	...
	aproxKnap(n, capacity, beta, objects);
